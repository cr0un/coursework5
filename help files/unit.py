...
class BaseUnit:
    def __init__(self, name: str, unit_class: str, weapon: Weapon, armor: Armor, max_health: int, max_stamina: int, skill: Optional[Skill] = None):
        self.name = name
        self.unit_class = unit_class
        self.weapon = weapon
        self.armor = armor
    ...
    def __str__(self):
        return f"{self.name} ({self.unit_class}): {self.health} health, {self.stamina} stamina"

    def _count_damage(self, target: 'BaseUnit') -> int:
        """
        функция рассчета урона:
        она используется для расчета урона нанесенного в результате удара.
        возвращаемое значение -- это количество урона.
        """
        pass
    ...
    def get_damage(self, damage: int) -> Optional[int]:
        ...
        здесь происходит проверка достаточно ли текущего здоровья для выживания.
        Если уровень здоровья меньше или равен 0, то возвращаем None.
        В противном случае мы вычитаем урон из текущего здоровья и возвращаем значение урона.
        """
        pass
...
class PlayerUnit(BaseUnit):
    ...
    def hit(self, target: 'BaseUnit') -> str:
        ...
        if damage > 0:
            ...
            if self.stamina < 0:
                self.stamina = 0
                return f"{self.name} не смог использовать {self.weapon.name}, у него закончилась выносливость."
            else:
